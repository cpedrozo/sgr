CREATE OR REPLACE VIEW sgr.vw_registros_dpto AS
SELECT r.id_registro, 
s.nombre ||' - '|| dp.nombre sucursal_dpto,
substring(te.nombre from 1 for 3) ||': '|| substring(e.nombre from 1 for 8) ||' - '|| wf.nombre tipoevento_y_wf,
            substring(r.nombre from 1 for 20)||'..' nombre, r.archivo_nombre, nu.nombre urgencia, c.caducidad,
